...
export default function PaymentFailed() {
  useEffect(() => {
    ...
    const redirectToApp = async () => {
      try {
        // Try to open the app with the failed parameter
        const canOpen = await Linking.canOpenURL('atletech://payment-callback?success=false');
        
        if (canOpen) {
          await Linking.openURL('atletech://payment-callback?success=false');
        } else {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.message}>
        Your payment could not be processed. You will be redirected back to the app.
      </Text>
